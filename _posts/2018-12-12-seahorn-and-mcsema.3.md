...
back-end solvers (aka engines) to solve VCs. The most powerful engines
use [Spacer](https://bitbucket.org/spacer/code)
and [Crab](https://github.com/seahorn/crab) to automatically infer
loop invariants if the VCs are expressed over quantified-free theory
of arrays and linear integer arithmetic.  These engines allow SeaHorn
to prove absence of errors, not just find bugs. Unfortunately, these
engines are not applicable in our context because they are not
appropriate for bit-precise analysis.
...
1.	For every function `f` in the original binary, we identify the
    ...
    because the binary transforms keep this mapping.

2.	Both functions `f` and `f’` are translated to LLVM bitcode using
    McSema as described in our
    previous
    [post](http://seahorn.github.io/seahorn/mcsema/equivalence/x86/binary/2018/12/12/seahorn-and-mcsema.2.html).
    In the function body, calls to other functions are modeled with
    uninterpreted functions. Calls to user-defined functions are
    replaced with an external function with the same name in the
    original and the variant function.
...
3.	We merge the bitcode of `f` and `f’` using the `llvm-link` tool and
    ...
    (`Mem_in_f == Mem_in_f'`), and after both functions return, we
    insert an assertion to check that the output of both functions is
    identical (`Mem_out_f == Mem_out_f'`).
...
defined as a pair of arrays: (`Reg_i x Mem_i`) for `i in {1,2}`.
...
Reg_i 7)`. We use index `i in {1,2}` to identify the two
...
`param_i_start` and `param_i_end`. This alternative encoding is more
...
We use McSema version 1 , LLVM version 3.8, and the latest version of
SeaHorn. We made some changes to McSema to prevent irrelevant ELF
sections from polluting the bitcode (e.g., the .eh_frame section), and
we wrote several Python scripts and LLVM passes to instrument the
McSema bitcode.
...
These are not fundamental limitations but we simply did not have time
to tackle them.
...
SeaHorn + Boolector, verification fails on 17 pairs of functions. Z3's
...
the most practical. Our goal was only to use SeaHorn's components as
they are whenever possible in order to see how far we could go. But,
there are many things that are worth investigating:

- apply static analysis 'a la' [value-set analysis](https://research.cs.wisc.edu/wpis/papers/cc04.pdf) for stack recovery.
- parallel symbolic execution to reason simultaneously about paths from different functions. 
- SMT heuristics to solve more efficient path conditions.
- better modeling of global variables.
- pointer analysis with more precise reasoning about `inttoptr` instructions.
- etc
